...
function onResults(results) {
  ...
  if (results.poseLandmarks) {
    ...
    const leftHipPos = {
        x: leftHip.x * canvasElement.width,
        y: leftHip.y * canvasElement.height,
    };
 
    const rightHipPos = {
        x: rightHip.x * canvasElement.width,
        y: rightHip.y * canvasElement.height,
    };
    ...
    if (rightHipAngle < 100 || rightKneeAngle < 135) {
      canvasCtx.fillStyle = 'lime';
      canvasCtx.font = '30px Arial';
      canvasCtx.fillText('Now Push Up', leftKneePos.x, leftKneePos.y - 10);
      canvasCtx.fillText('Now Push Up', leftHipPos.x, leftHipPos.y - 10);
    }
    else if (rightKneeAngle > 165 || rightHipAngle > 165){
      canvasCtx.fillStyle = 'lime';
      canvasCtx.font = '30px Arial';
      canvasCtx.fillText('Ok, Now Push Down', rightKneePos.x, rightKneePos.y - 10);
      canvasCtx.fillText('Ok, Now Push Down', rightHipPos.x, rightHipPos.y - 10);
    }
    else{
      canvasCtx.fillStyle = 'lime';
      canvasCtx.font = '30px Arial';
